...
class OfferController extends Controller
{
    ...
    public function postAction(Request $request)
    {
        ...
        return $this->render('ProBundle:Form:postOffer.html.twig', array(
    ...
    public function editAction(Request $request)
    {
        ...
        return $this->render('ProBundle:Form:editOffer.html.twig', array(
            'form' => $form->createView(),
        ));
    }
    ...
    public function deleteAction(Request $request){
        ...
        ;
        $employer = $employerRepository->findOneBy(array('id' => $user->getPro()));
    ...
    public function showAction(Request $requestPrime, $id){
        ...
        $seoPage
            ...
            ->addMeta('property', 'og:image', 'https://rendezvous.lu/uploads/images/employer/'.$offer->getPro()->getLogo()->getImageName())
            ->addMeta('property', 'og:description', $offer->getDescription())
            ->addMeta('property', 'og:url', $requestPrime->getUri());

        return $this->render('ProBundle:Offer:show.html.twig', array(
            'offer' => $offer,
    ...
    public function activateAction(Request $request){
        ...
        foreach ($ids as $id){
            $offer = $offerRepository->findOneBy(array('id' => $id));
            if(!isset($user) || !in_array('ROLE_EMPLOYER', $user->getRoles()) || $offer->getPro()->getId() != $employer->getId()){
                $translated = $this->get('translator')->trans('redirect.employer');
                $session->getFlashBag()->add('danger', $translated);
                return $this->redirectToRoute('create_employer');
            }
        ...
        $employer->setCredit($creditPro - $creditOffer);
...
//        $map->setMapOption('zoom', 2);
    ...
    public function searchPageAction(Request $request){
        $keywords = $request->get('keyword');
        $location = $request->get('location');
        $chosenPro = $request->get('employer');
    ...
    public function boostAction(Request $request){
        ...
        if($creditPro < $creditBoost){
            $translated = $this->get('translator')->trans('form.offer.boost.error');
            $session->getFlashBag()->add('danger', $translated);
            return $this->redirectToRoute('rendezvous_credit');
        }

        $employer->setCredit($creditPro - $creditBoost);
    ...
    public function applyAction(Request $request){
        ...
        $translatedPro = $this->get('translator')->trans('offer.applied.employer');

        $messageEmmployer = (new \Swift_Message($translatedPro . ' ' . $offer->getTitle()))
            ->setFrom('rendezvouslu@noreply.lu')
            ->setTo($firstUser)
            ->setCc(array_shift($arrayEmail))
            ->setBody(
        ...
        $translatedClient = $this->get('translator')->trans('offer.applied.candidate');
        $messageClient = (new \Swift_Message($translatedClient . ' ' . $offer->getTitle()))
            ->setFrom('rendezvouslu@noreply.lu')
            ->setTo($candidateMail)
            ->setBody(
                $this->renderView(
        ...
        $postulatedOffer->setClient($candidate);
    ...
    public function offerNotFoundAction($id)
    {
        ...
        return $this->render('ProBundle:Offer:offerNotFound.html.twig', array(
            'similarOfferArray' => $similarOfferArray['offers'],
            'tags' => $similarOfferArray['tags'],
        ));
    }
    ...
    public function sendEndActivationAction(Request $request, $days){
        ...
        if(!empty($offers)){
            ...
            foreach ($offers as $offer){
                ...
                $userArray = $userRepository->findBy(array('employer' => $offer->getPro()));
